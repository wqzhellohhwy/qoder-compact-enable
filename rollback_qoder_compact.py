# -*- coding: utf-8 -*-
"""
Qoder 对话压缩 patch 回滚脚本
从最近一次 .bak_compact_* 备份恢复 agents-window.desktop.main.js。
用法：右键本 bat -> 以管理员身份运行
...
def main():
    ...
    if not baks:
        print("[*] 未找到 compact 备份，无需回滚（或备份已被清理）。")
        return
    ...
    if not any(m in current for m in markers):
        print("[*] 当前文件已无 compact patch 标记，无需回滚。")
        return

    # 当前状态备份（防手滑）
    ts = os.path.basename(bak).replace(KEY, "")
    pre = JS + ".bak_compact_reverted_" + ts
    shutil.copy2(JS, pre)
    print("[2/3] 已备份当前状态:", os.path.basename(pre))

    shutil.copy2(bak, JS)
    print("[3/3] 已从备份恢复。重启 Qoder 后压缩门槛恢复原样（40%）。")
